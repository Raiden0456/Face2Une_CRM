...
package_p.buyPackages = async (
  ...
  return result(null, {
    message: "Packages bought successfully",
    promocodes: all_promocodes,
  });
};

// Using packages promocodes //
package_p.usePackage = async (client_id: number, promocode: string, result) => {
  ...
  if (current.data.length > 0) {
    if (
      current.data[0].amount_left_in > 0 &&
      current.data[0].expiry_date >= formatted_current_date
    ) {
